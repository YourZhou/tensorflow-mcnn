...
def read_train_data(img_path, gt_path, crop_size=256, scale=8, knn_phase=True, k=2, min_head_size=16,
                    max_head_size=200):
    """
...
if __name__ == '__main__':
    crowd_img, density_map, cropped_crowd_count = read_test_data(
        'D:\\Scenic_file\\Scenic_dataset\\Scenic_photo\\shop3\\photo\\Scenic_170.jpg',
        'D:\\Scenic_file\\Scenic_dataset\\Scenic_photo\\shop3\\mat\\GT_Scenic_170.mat',
        scale=8, deconv_is_used=False, knn_phase=True, k=20, min_head_size=16,
        max_head_size=200
    )
    ...
    # points = label_data['image_info'][0][0]['location'][0][0]
    #
    # dmp = get_density_map([ori_crowd_img.shape[0], ori_crowd_img.shape[1]], points, knn_phase=False, k=1,
    #                       scaled_min_head_size=2, scaled_max_head_size=3)
    # print(dmp.shape)
    ...
    # rgb_img = cv.imread('D:\\YourZhouProject\\mcnn_project\\pytorch_mcnn\\part_A_final\\train_data\\images\\IMG_50.jpg')
    sum = np.sum(np.sum(density_map))
    print(sum, cropped_crowd_count)
    print(crowd_img.shape)
    # rea_img = (rgb_img - 127.5) / 128
    show_density_map(density_map[0, :, :, 0])
    # show_density_map(crowd_img[0, :, :, 1])
    cv.imshow("123", crowd_img[0, :, :, :])
